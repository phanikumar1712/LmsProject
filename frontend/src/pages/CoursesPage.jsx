                ...
                <div className="bg-card border border-border rounded-2xl p-6 sm:p-8 mb-8 shadow-sm text-card-foreground">
                    <div className="flex flex-col md:flex-row gap-8">
                        ...
                        <div className="flex-1 min-w-48">
                            <p className="text-[11px] font-bold text-muted-foreground/60 uppercase tracking-wider mb-4">Category</p>
                            <div className="flex flex-wrap gap-2.5">
                                {categories.map(cat => (
                                    <button
                                        key={cat.id}
                                        onClick={() => updateFilter('category', cat.id)}
                                        className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${filters.category === cat.id ? 'bg-indigo-600 text-white shadow-sm' : 'bg-muted text-muted-foreground hover:bg-muted/80 border border-border/50'}`}
                                    >
                                        {cat.icon} {cat.name}
                                    </button>
                                ))}
                        ...
                        <div className="md:border-l md:border-border/50 md:pl-8">
                            <p className="text-[11px] font-bold text-muted-foreground/60 uppercase tracking-wider mb-4">Level</p>
                            <div className="flex gap-2.5 flex-wrap">
                                {LEVELS.map(level => (
                                    <button
                                        key={level}
                                        onClick={() => updateFilter('level', level)}
                                        className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${filters.level === level ? 'bg-indigo-600 text-white shadow-sm' : 'bg-muted text-muted-foreground hover:bg-muted/80 border border-border/50'}`}
                                    >
                                        {level}
                                    </button>
                                ))}
                    ...
                    {hasActiveFilters && (
                        <div className="mt-8 pt-5 border-t border-border/50 flex justify-end">
                            <button onClick={clearFilters} className="text-sm text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:hover:bg-rose-950/20 px-4 py-2 rounded-lg font-semibold flex items-center gap-1.5 transition-colors">
