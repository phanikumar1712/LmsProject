    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {(categories ?? []).map(cat => (
                    <div key={cat.id} className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col items-center text-center shadow-sm hover:shadow-md transition-shadow">
                        <div className="text-4xl mb-3">{cat.icon}</div>
                        <h3 className="font-bold text-slate-900 mb-1">{cat.name}</h3>
                        <p className="text-xs text-slate-500 font-bold uppercase mb-4">{cat.courseCount} Courses</p>

                        <div className="flex items-center gap-2 w-full pt-4 border-t border-slate-100">
                            <button
                                aria-label={`Edit ${cat.name}`}
                                onClick={() => handleEdit(cat)}
                                className="flex-1 py-1.5 flex justify-center text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 rounded-lg transition-colors"
                            >
                                <Edit2 size={14} />
                            </button>
                            <button
                                ...
                                className="flex-1 py-1.5 flex justify-center text-slate-400 hover:text-rose-600 bg-slate-50 hover:bg-rose-50 rounded-lg transition-colors"
                            >
                                <Trash2 size={14} />
                            </button>
                        </div>
