...
export default function InstructorReviews() {
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <div>
                <h1 className="text-2xl font-extrabold text-slate-900">Student Reviews</h1>
                <p className="text-slate-500 text-sm font-medium mt-1">Respond to feedback on your courses</p>
            </div>

            {reviews.length === 0 ? (
                <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center">
                    <MessageSquare size={40} className="mx-auto text-slate-300 mb-4" />
                    <p className="text-slate-500 font-medium">No reviews yet on your courses.</p>
                </div>
            ) : (
                        ...
                        <div key={review.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                            ...
                            {review.instructorReply && (
                                <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 mb-4">
                                    <p className="text-xs font-bold text-indigo-600 uppercase mb-1">Your reply</p>
                                    <p className="text-sm text-slate-700">{review.instructorReply}</p>
                                </div>
                            )}
                            ...
                            {replyingTo === review.id ? (
                                <div className="space-y-3">
                                    <textarea
                                        ...
                                        className="w-full border border-slate-200 rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-indigo-100"
                                        placeholder="Write your reply..."
                                        rows={3}
                                    />
                                    <div className="flex gap-2">
                                        <button onClick={() => handleReplySubmit(review.id)} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-bold">Send Reply</button>
                                        <button onClick={() => { setReplyingTo(null); setReplyText(''); }} className="text-slate-500 px-4 py-2 text-sm font-bold">Cancel</button>
                                    </div>
                                </div>
                            ) : (
                                <button
