...
export default function InstructorQuizBuilder() {
    ...
    const InputClass = "w-full bg-white border border-slate-200 text-slate-900 placeholder:text-slate-400 rounded-xl py-3 px-4 focus:ring-2 focus:ring-indigo-100 outline-none shadow-sm transition-shadow text-[15px]";

    return (
        <div className="max-w-4xl mx-auto space-y-6 pb-20">
            <div>
                <h1 className="text-3xl font-extrabold text-slate-900 mb-2 tracking-tight">Quiz Builder</h1>
                <p className="text-slate-500 font-medium">Create powerful assessments with varying answer types and anti-cheat enforcements.</p>
            </div>

            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-8 space-y-5">
                <h2 className="text-xl font-bold text-slate-900 border-b border-slate-100 pb-3">Quiz Details</h2>
                <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                        <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wide block mb-2">Target Course</label>
                    ...
                    <div>
                        <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wide block mb-2">Quiz Title</label>
                    ...
                    <div>
                        <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wide block mb-2">Time Limit (mins)</label>
                        <input type="number" name="timeLimit" value={quizInfo.timeLimit} onChange={handleInfoChange} className={InputClass} min="1" />
                    </div>
                    <div>
                        <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wide block mb-2">Passing Score (%)</label>
                    ...
                    <div className="sm:col-span-2">
                        <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wide block mb-2">Instructions</label>
            ...
            <div className="space-y-5">
                {questions.map((q, qIdx) => {
                    ...
                    return (
                        <div key={q.id} className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden transition-all">
                            <div className="bg-slate-50 border-b border-slate-200 p-4 flex items-center gap-3">
                                ...
                                <QIcon size={18} className="text-slate-500" />
                                <span className="text-slate-700 font-bold text-[14px] flex-1">{QUESTION_TYPES.find(t => t.value === q.type)?.label}</span>
                                <button onClick={() => updateQuestion(qIdx, 'isExpanded', !q.isExpanded)} className="p-2 text-slate-400 hover:bg-slate-200 hover:text-slate-700 rounded-lg transition-colors"><ChevronDown size={18} className={q.isExpanded ? "rotate-180 transition-transform" : "transition-transform"} /></button>
                            ...
                            {q.isExpanded && (
                                <div className="p-6 space-y-5">
                                    <div>
                                        <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wide block mb-2">Question Prompt *</label>
                                    ...
                                    {(q.type === 'MCQ_SINGLE' || q.type === 'MCQ_MULTI') && (
                                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-4">
                                            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Options & Correct Answer</label>
                                            {q.options.map((opt, oIdx) => (
                                                <div key={oIdx} className="flex items-center gap-3">
                                                    <button onClick={() => toggleCorrectOption(qIdx, oIdx)} className={`w-7 h-7 flex items-center justify-center rounded-lg transition-colors flex-shrink-0 shadow-sm ${q.correctAnswers.includes(oIdx) ? 'bg-emerald-500 text-white border-emerald-600' : 'bg-white border border-slate-300 text-transparent hover:border-indigo-400'}`}>
                                                        {q.correctAnswers.includes(oIdx) && <CheckCircle size={16} />}
                                                    </button>
                                                    <input type="text" value={opt} onChange={e => updateOption(qIdx, oIdx, e.target.value)} className="flex-1 bg-white border border-slate-200 rounded-lg py-2.5 px-3 text-[14px] font-bold text-slate-900 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 shadow-sm" placeholder={`Option ${oIdx + 1}`} />
                                                    <button onClick={() => removeOption(qIdx, oIdx)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                                                </div>
                                            ))}
                                    ...
                                    {q.type === 'FILL_BLANK' && (
                                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                                            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Acceptable Answer(s) (Case-insensitive)</label>
            ...
            <div className="flex flex-wrap gap-3 pt-5">
                {QUESTION_TYPES.map(qt => (
                    <button key={qt.value} onClick={() => addQuestion(qt.value)} className="px-4 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 hover:border-slate-300 rounded-xl text-[13px] font-bold text-slate-700 flex items-center gap-2 shadow-sm transition-all focus:ring-2 focus:ring-indigo-100">
                        <Plus size={16} className="text-indigo-600" /> Add {qt.label}
                    </button>
                ))}
            </div>

            <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md p-4 border-t border-slate-200 z-20 flex justify-end px-8 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)]">
                <div className="w-full max-w-4xl mx-auto flex justify-end gap-3">
                    <button onClick={() => navigate(-1)} className="px-6 py-2.5 rounded-xl text-[14px] font-bold text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 transition-colors">Cancel</button>
